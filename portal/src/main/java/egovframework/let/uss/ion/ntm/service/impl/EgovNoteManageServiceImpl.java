package egovframework.let.uss.ion.ntm.service.impl;
...
public class EgovNoteManageServiceImpl extends AbstractServiceImpl   implements EgovNoteManageService {
    ...
    public Map selectNoteManage(NoteManage noteManage) throws Exception {
    	return (Map)dao.selectNoteManage(noteManage);
    }
     ...
     * @throws Exception
     */
    public void insertNoteManage(NoteManage noteManage, Map commandMap)throws Exception{
  
        /* ****************************************************************
         * 쪽지관리 처리
         **************************************************************** */
    	//쪽지 ID설정
    	noteManage.setNoteId((String)noteIdgenService.getNextStringId());
    	//쪽지 등록
         ...
         **************************************************************** */
    	//보낸쪽지함 ID설정
    	noteManage.setNoteTrnsmitId((String)noteTrnsmitIdgenService.getNextStringId());
    	//발신자 아이디설정
        ...
        for(int i=0;i<sRecptnEmpListResult.length;i++){
        	...
        	//받은쪽지함 ID설정
        	noteManage.setNoteRecptnId((String)noteRecptnIdgenService.getNextStringId());
        	//받은쪽지함 수신여부 설정
	...
	public int selectNoteEmpListPopupCnt(ComDefaultVO searchVO) throws Exception{
		return (Integer)dao.selectNoteEmpListPopupCnt(searchVO);
	}
}
